...
public class GoogleProvider implements LLMProvider {
    ...
    public LLMResponse chat(LLMRequest request) {
        try {
            ...
            String maskedKey = config.getApiKey().substring(0, 4) + "..."
                    + config.getApiKey().substring(config.getApiKey().length() - 4);
            System.out.println("DEBUG: Calling Google API URL: " + baseUrl
                    + String.format("/models/%s:generateContent?key=%s", model, maskedKey));

...
class NoOpAuditLoggerTest {
    ...
    void testLogToolExecution_noSideEffects() {
        ...
        assertDoesNotThrow(() -> logger.logToolExecution("session", "tool", "input", "output", Instant.now()));
    }
    ...
    void testLogPromptUsage_noSideEffects() {
        ...
        assertDoesNotThrow(() -> logger.logPromptUsage("session", "prompt-id", "v1", Instant.now()));
    }
    ...
    void testLogConversationEvent_noSideEffects() {
        ...
        assertDoesNotThrow(() -> logger.logConversationEvent("session", "user", "TEST_EVENT", Map.of("key", "value")));
    }
    ...
    void testPerformance_noOverhead() {
        ...
        assertTrue(duration < 10_000_000,
                "NoOpAuditLogger should have near-zero overhead, took: " + (duration / 1_000_000) + "ms");
    }
}

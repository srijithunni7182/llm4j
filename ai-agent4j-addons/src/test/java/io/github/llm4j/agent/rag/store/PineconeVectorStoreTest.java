...
class PineconeVectorStoreTest {
    ...
    void add_shouldUpsertToPinecone() {
        ...
        verify(mockIndex).upsert(idCaptor.capture(), valuesCaptor.capture(), any(), any(), metadataCaptor.capture(), eq("test-namespace"));
    ...
    void search_shouldQueryAndReturnResults() {
        ...
        when(match.getMetadata()).thenReturn(Struct.newBuilder().putFields("text", Value.newBuilder().setStringValue("content").build()).build());
        ...
        verify(mockIndex).query(eq(5), anyList(), any(), any(), any(), eq("test-namespace"), any(), eq(false), eq(true));
    }
    ...
    void delete_shouldDeleteFromIndex() {
        ...
        verify(mockIndex).delete(eq(Collections.singletonList("vec1")), eq(false), eq("test-namespace"), any());
    }
}

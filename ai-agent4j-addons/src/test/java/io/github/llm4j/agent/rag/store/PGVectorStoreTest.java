...
public class PGVectorStoreTest {
    ...
    private void initVectorStore() {
        vectorStore = new PGVectorStore("jdbc:postgresql://localhost:5432/test", "user", "pass", "embeddings", 384) {
            @Override
    ...
    void testAdd_shouldInsertVector() throws SQLException {
        ...
        verify(mockPreparedStatement).setObject(eq(2), any(PGvector.class)); // Can't easily match PGvector equality without equals
    ...
    void testSearch_shouldReturnResults() throws SQLException {
        ...
        // Mock search result
        // when(mockStatement.executeQuery(anyString())).thenReturn(mockResultSet); // Not used in search

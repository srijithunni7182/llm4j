...
public class LanguageMapper {
    ...
    private static void loadLanguages() {
        try (InputStream is = LanguageMapper.class.getResourceAsStream("/sarvam_languages.json")) {
            if (is == null) {
                logger.warn(
                        "sarvam_languages.json not found in resources. Language mapping will be empty.");
                return;
            }
    ...
    /**
     * Resolves a language name or code to a valid Sarvam language code. Case-insensitive.
     *
     * @param input the language name (e.g., "Malayalam") or code (e.g., "ml-IN")
     * @return the corresponding language code, or the input if not found in the map (assuming it
